...
reusable pipeline lives in [ShiplightAI/ci-triage]. Pin release tags such as
`@v1.2`; the upstream `release-tags` ruleset prevents updates and deletion of
published `v*` tags.
...
- `autofix-runner` re-runs tests and needs browsers and network. Use a Shiplight
  runner or another runner image with Chromium already installed. In v1.2 the
  reusable workflow has no caller hook that installs Chromium; plain `ubuntu-latest`
  is unsupported for autofix.
